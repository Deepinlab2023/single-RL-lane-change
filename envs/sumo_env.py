...
class SumoEnv(gym.Env):
    ...
    def __init__(self, gui=False, step_length=0.1, L1=20, L2=50,
                 ...
                 safe_distance=5.0, route_distance=500,
                 w_collision=1.0, w_completion=1.0, w_lane_change=1.0, **kwargs):
        super().__init__()
    ...
    def reset(self, seed=None, options=None):
        ...
        try:
            ...
            # Initialize y_target to current lateral position
            ego_lane = self.conn.vehicle.getLaneIndex(self.ego_id)
            self.y_target = self._get_lane_centerline(ego_lane)
    ...
    def step(self, action):
        ...
        try:
            ...
            self.termination_reason = "TRACI_ERROR"
            info = self._build_info_dict()
            return self._get_observation(), 0.0, True, False, info

        # Update target velocity based on action
        if action == 0:
            self.v_target = min(self.v_max, self.v_target + self.delta_t)
        elif action == 1:
            self.v_target = max(0, self.v_target - self.delta_t)
        elif action == 2:
            pass

        # Update target lateral position based on action
        if action == 3:
            if current_lane > 0:
                self.y_target = self._get_lane_centerline(current_lane - 1)
        elif action == 4:
        ...
        for _ in range(self.decision_steps):
            ...
            # Apply acceleration to ego vehicle
            current_speed = self.conn.vehicle.getSpeed(self.ego_id)
            new_speed = max(0, min(self.v_max, current_speed + a_e * self.step_length))
            self.conn.vehicle.setSpeed(self.ego_id, new_speed)
            
            # Compute lateral control using lateral controller
            target_lane, tau = self.lateral_controller()
            
            # Apply lane change if needed
            if target_lane is not None and tau is not None:
                try:
                    self.conn.vehicle.changeLane(self.ego_id, target_lane, tau)
                except traci.TraCIException:
                    pass
    ...
    def _build_info_dict(self):
        """Build info dictionary with per-episode metrics"""
        info = {
            'collision': self.collision_occurred,
            'termination_reason': self.termination_reason if self.termination_reason else "ONGOING"
        }

        # Calculate average speed for this episode
        if self.episode_speed_count > 0:
            avg_speed = self.episode_speed_sum / self.episode_speed_count
            info['average_speed_m_s'] = avg_speed
            info['average_speed_km_h'] = avg_speed * 3.6
        else:
            info['average_speed_m_s'] = 0.0
            info['average_speed_km_h'] = 0.0

        # Calculate distance travelled for this episode
        if self.episode_start_position is not None:
            try:
                current_position = self.conn.vehicle.getLanePosition(self.ego_id)
                distance_travelled = abs(current_position - self.episode_start_position)
                info['distance_travelled_m'] = distance_travelled
            except traci.TraCIException:
                info['distance_travelled_m'] = 0.0
        else:
            info['distance_travelled_m'] = 0.0
    ...
    def _compute_reward(self):
        """Compute reward signal with configurable weights"""
        ...
        try:
            ...
            # Collision penalty (weight * value)
            if collision_occurred:
                reward += self.w_collision * (-5.0)

            # Route completion bonus (weight * value)
            if self.route_end and not collision_occurred:
                reward += self.w_completion * 5.0

            # Lane change penalty (weight * value)
            if lane_changed:
                reward += self.w_lane_change * (-1.0)

            # Efficiency reward based on speed (weight * value)
            v_min = 0.0
            efficiency_reward = -1.0 + 2.0 * (current_speed - v_min) / (self.v_max - v_min)
            reward += self.w_efficiency * efficiency_reward
    ...
    def _spawn_random_vehicles(self, num_vehicles):
        """Spawn initial vehicle distribution"""
        vehicle_count = 0
        spawned_positions = []
        safe_spawn_clearance = 15

        spawn_start = 100
        spawn_end = min(1000, self.edge_length - 100)
    ...
    def render(self, mode='human'):
        """Pygame rendering with scrolling effect"""
        ...
        try:
            ...
            pygame.draw.rect(self.screen, GRASS_GREEN,
                             (0, road_top + road_height, self.screen_width, self.screen_height))
            ...
            elif self.termination_reason == "ROUTE_COMPLETED":
                title = self.font_large.render("ROUTE COMPLETED", True, (0, 255, 0))
            elif self.termination_reason == "TRACI_ERROR":
    ...
    def PID(self):
        """PID controller for ego vehicle longitudinal control"""
        v_current = self.conn.vehicle.getSpeed(self.ego_id)
        
        error = self.v_target - v_current
        
        self.integral_error += error * self.dt_control
        
        derivative_error = (error - self.previous_error) / self.dt_control
        ...
        a_e = np.clip(a_e, -4.0, 3.0)
        
        return a_e

    def lateral_controller(self):
        """Lateral controller using SUMO's changeLane command with velocity-based tau"""
        try:
            ego_x, ego_y = self.conn.vehicle.getPosition(self.ego_id)
            v_e = self.conn.vehicle.getSpeed(self.ego_id)
            current_lane = self.conn.vehicle.getLaneIndex(self.ego_id)
            
            if v_e < 0.1:
                v_e = 0.1
            
            delta_y = self.y_target - ego_y
            
            arcsin_arg = np.clip(self.k_delta_y * delta_y / v_e, -1.0, 1.0)
            psi_r = self.k_delta_y * np.arcsin(arcsin_arg)
            
            delta_e_arg = np.clip((self.l_f + self.l_r) / (2 * v_e) * psi_r, -1.0, 1.0)
            delta_e = np.arcsin(delta_e_arg)
            
            v_y = v_e * np.sin(delta_e)
            
            lateral_distance = self.lane_width
            tau = lateral_distance / abs(v_y) if abs(v_y) > 0.01 else 3.0
            
            tau = np.clip(tau, 1.0, 5.0)
            
            target_lane = int(self.y_target / self.lane_width)
            target_lane = max(0, min(self.num_lanes - 1, target_lane))
            
            if target_lane == current_lane:
                return None, None
            
            return target_lane, tau
            
        except traci.TraCIException:
            return None, None

    def _get_lane_centerline(self, lane_index):
        """Calculate the y-coordinate of a lane's centerline"""
        return lane_index * self.lane_width + self.lane_width / 2.0
